...
class worker_node(Node):
    ...
    def execute_blocks(self):
        ...
        # Block Name: takeoff high
        start_time = 0.040000000000000036
        self.timeHelper.sleepUntil(start_time)
        takeoff(groupState, 1.6, 3)
        goto_velocity_relative_position(groupState, -0.5,0,0,0.25)
        # Block Name: vertical
        start_time = 6.626667480468747
        self.timeHelper.sleepUntil(start_time)
        goto_velocity_relative_position(groupState, 0,0,-0.1,0.25)
        goto_velocity_relative_position(groupState, 0,0,0.1,0.25)
        goto_velocity_relative_position(groupState, 0,0,-0.2,0.25)
        goto_velocity_relative_position(groupState, 0,0,0.2,0.25)
        goto_velocity_relative_position(groupState, 0,0,-0.3,0.25)
        goto_velocity_relative_position(groupState, 0,0,0.3,0.25)
